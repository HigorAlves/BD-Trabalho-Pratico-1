...
const CONSULTA1 = 'select sum(valor) from recursos_financeiros where DataCad BETWEEN (?) AND (?);'
const CONSULTA2 = 'select ga.NomeGA, sum(r.valor) from recursos_financeiros inner join Grandes_Areas ga on ga.IdGA = (?) where DataCad BETWEEN (?) AND (?) group by ga.NomeGA'
const CONSULTA3 = 'select a.NomeArea, sum(r.valor) from recursos_financeiros r inner join Areas a on a.idArea = (?) where DataCad BETWEEN (?) AND (?) group by a.NomeArea'
